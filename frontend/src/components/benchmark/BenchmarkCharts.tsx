...
import { quantumRuntimeMs } from '../../lib/benchmark'
...
export function BenchmarkCharts({ document }: BenchmarkChartsProps) {
  const classical = document.classical
  const ratio = document.approximationRatio.value
  const feasible = document.constraintViolations.length === 0
  const classicalRuntime = classical.runtimeMs
  const qaoaRuntime = quantumRuntimeMs(document)
  ...
  return (
    <section className="grid grid-cols-1 gap-4 lg:grid-cols-2" aria-label="Benchmark charts">
      <PairChart
        ...
        data={[{ name: 'Wall time', classical: classicalRuntime, qaoa: qaoaRuntime }]}
        formatValue={(value: number | null) => (value === null ? 'not recorded' : formatDuration(value))}
        note={
          qaoaRuntime === null
            ? 'QAOA executor wall time was not persisted — see the metric table for the pipeline total.'
            : undefined
        }
      />
      <PairChart
        ...
        data={[
          {
            name: 'Objective',
            classical: classical.objectiveValue !== null ? classical.objectiveValue * 100 : null,
            qaoa: document.quantum.objectiveValue !== null ? document.quantum.objectiveValue * 100 : null,
          },
        ]}
        formatValue={(value: number | null) => (value === null ? 'not recorded' : `${value.toFixed(2)}%`)}
      />
      <section className="glass-card p-5" aria-label="Solution quality chart">
        ...
        <p className="mt-0.5 text-[11px] text-mist-500">
          Stored approximation ratio = QAOA objective ÷ classical objective. The amber line is parity (1.0).
        </p>
      ...
      <section className="glass-card p-5" aria-label="Constraint violations chart">
        ...
        <p className="mt-0.5 text-[11px] text-mist-500">
          Violations stored for the QAOA decode. The classical reference is feasible by construction (0 stored).
        </p>
        <div role="img" aria-label="Constraint violations recorded for this experiment." style={{ height: 300 }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={[{ name: 'Constraint violations', value: document.constraintViolations.length }]} margin={{ top: 24, right: 16, bottom: 4, left: 8 }}>
              <CartesianGrid stroke={chartColors.grid} strokeDasharray="3 3" />
              <XAxis dataKey="name" tick={{ fill: chartColors.tick, fontSize: 11 }} tickLine={false} axisLine={false} />
...
interface PairChartProps {
  title: string
  ariaLabel: string
  data: { name: string; classical: number | null; qaoa: number | null }[]
  formatValue: (value: number | null) => string
  note?: string
}

function PairChart({ title, ariaLabel, data, formatValue, note }: PairChartProps) {
  return (
    <section className="glass-card p-5" aria-label={title}>
      ...
      <div role="img" aria-label={ariaLabel} style={{ height: 300 }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 4, right: 16, bottom: 4, left: 8 }} barCategoryGap={4}>
            <CartesianGrid stroke={chartColors.grid} strokeDasharray="3 3" />
            <XAxis dataKey="name" tick={{ fill: chartColors.tick, fontSize: 11 }} tickLine={false} axisLine={false} />
            <YAxis tick={{ fill: chartColors.tick, fontSize: 10 }} tickLine={false} axisLine={false} width={90} />
            <Tooltip contentStyle={tooltipStyle} formatter={(value, name) => [formatValue(Number.isFinite(Number(value)) ? Number(value) : null), name]} />
      ...
      {note !== undefined && <p className="mt-2 text-[11px] text-mist-500">{note}</p>}
    </section>
  )
}